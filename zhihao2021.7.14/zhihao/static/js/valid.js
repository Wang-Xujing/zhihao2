...
$(function () {
    ...
    monitor_live_num();
    if (model_name === 'feedingin') {
        monitor_feedingin_total_price();
    }
    else if(model_name === 'vaccine_in'){
        monitor_vaccinein_total_price()
    }

})
;

function monitor_feedingin_total_price() {
    var $unit_price = $('#id_unit_price');
    var $quantity = $('#id_quantity');
    var $total_price = $('#id_total_price');
    var $fare = $('#id_fare');
    var $avg_price = $('#id_avg_price');
    ...
    function count() {
        ...
        var total_price=parseFloat(unit_price) * parseFloat(quantity) + parseFloat(fare);
        $total_price.val(total_price);
        $avg_price.val(total_price/parseFloat(quantity));
    }
...
function monitor_vaccinein_total_price() {
    var $unit_price = $('#id_unit_price');
    var $in_amount = $('#id_in_amount');
    var $total_price = $('#id_total_price');
    var $fare = $('#id_fare');
    var $avg_price = $('#id_avg_price');
    ...
    function count() {
        var unit_price = $unit_price.val().trim();
        var in_amount = $in_amount.val().trim();
        var fare = $fare.val().trim();

        var total_price=parseFloat(unit_price) * parseFloat(in_amount) + parseFloat(fare);
        $total_price.val(total_price);
        $avg_price.val(total_price/parseFloat(in_amount));
    }
    ...
    $input.change(function () {
        ...
        if (value < 6) {
            ...
            for (var i = 1; i <= value; i++) {
                ...
                div_basic.after('<div class="form-row field-color' + String(i) + ' add-after"><div><label for="id_color' + String(i) + '">羔羊毛色' + String(i) + ':</label><select name="color' + String(i) + '" id="id_color' + String(i) + '"><option value="" selected="">---------</option><option value="0">白</option><option value="1">黑</option><option value="2">咖</option><option value="3">混合色</option></select>');

                div_basic.after('<div class="form-row field-rank' + String(i) + ' add-after"><div><label for="id_rank' + String(i) + ' ">羔羊等级' + String(i) + ':</label><select name="rank' + String(i) + '" id="id_rank' + String(i) + '"><option value="" selected="">---------</option><option value="0">特级</option><option value="1">一级</option><option value="2">二级</option><option value="3">三级</option><option value="9">未评级</option></select>');

                div_basic.after('<div class="add-after">羔羊 ' + String(i) + '</div>')
            }
        } else {
...
function check() {
    ...
    const valid_fields = {
        ...
        'vaccine_out': ['outbound_no', 'v_name', 'out_staff', 'contact_phone', 'num'],
        'feedingin': ['f_name', 'buy_time', 'billing_unit', 'quantity', 'unit_price', 'f_date'],
        'feeding_out': ['outbound_no', 'f_name', 'out_staff', 'contact_phone', 'num'],
        'breederconditioninfo': ['mon_age', 'color', 'rank'],
        'cutinfo': ['cut_time', 'rank', 'color'],
        'v_suppliersinfo': ['supplier_name', 'address', 'f_date'],
