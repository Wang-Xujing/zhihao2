...
class breedingInfo(models.Model):
    ...
    ewe_choice=((0, '湖羊'), (1, '小尾寒羊'), (2, '无角道赛特'), (3, '道寒杂交F1'), (4, '白杜泊'), (5, '白萨福克'), (6, '杜湖杂交'),(7,'萨湖杂交'),(8,'杜寒杂交'),(9,'萨寒杂交'),(10,'道湖'))
    ...
    ram_choice=((0, '湖羊'), (1, '小尾寒羊'), (2, '无角道赛特'), (3, '道寒杂交F1'), (4, '白杜泊'), (5, '白萨福克'), (6, '杜湖杂交'),(7,'萨湖杂交'),(8,'杜寒杂交'),(9,'萨寒杂交'),(10,'道湖'))
    ...
    ##1.5号添加字段
    mat_period=models.IntegerField(verbose_name='配种情期(天)',blank=True,null=True)
    single_ok=models.CharField(verbose_name='单次配种成功率(%)',max_length=16,blank=True,null=True)
    ###
    ...
    def save(self, force_insert=False, force_update=False, using=None,
             update_fields=None):
        self.pre_production_date = self.breeding_date + datetime.timedelta(days=150)
...
class pregnantInfo(models.Model):
    ...
    f_staff=models.CharField(verbose_name='创建人员',blank=True,null=True,max_length=20)###默认为孕检人员关联表
    ...
    def ele_num(self):
        ewe=breedingInfo.objects.filter(id=self.breeding_id).first()
        if ewe:
            basic = BasicInfo.objects.filter(id=ewe.ewe_id).first()
            return basic.ele_num if basic else '-'
        else:
            return '-'
...
class export_breed(models.Model):
    breeding_id=models.IntegerField(verbose_name='配种信息id',blank=True,null=True)
    belong = models.IntegerField(verbose_name='羊场域')
    class Meta:
