        ...
        if request.method == "POST":
            ...
            try:
                if len(request_copy['many-basic_basicinfo_ewe']) > 0:
                    ...
                    request_copy['ewe_id'] = BasicInfo.objects.filter(name=request_copy['basic_basicinfo_ewe']).values('id').first()['id']
                else:
                    pass
            ...
            try:
                if len(request_copy['many-basic_basicinfo_ram']) > 0:
                    ...
                    request_copy['ram_id'] = BasicInfo.objects.filter(name=request_copy['basic_basicinfo_ram']).values('id').first()['id']
                else:
                    pass
            except MultiValueDictKeyError:
                pass
            print(BasicInfo.objects.filter(id=int(request_copy['ewe_id'])).first())
            request_copy['ewe_variety']=BasicInfo.objects.filter(id=int(request_copy['ewe_id'])).values('variety').first()['variety']
            request_copy['ram_variety'] = BasicInfo.objects.filter(id=int(request_copy['ram_id'])).values('variety').first()['variety']
    ...
    def _changeform_view(self, request, object_id, form_url, extra_context):
        ...
        ####
            ...
            for i in range(1, n + 1):
                from apps.basic.models import BasicInfo
                basic=BasicInfo.objects.filter(id=request.POST['ewe_id']).values().first()
                logo = basic['ele_num'] + '_' + request.POST['delivery_date'] + '_' + str(i)
                LambInfo.objects.create(logo=logo,belong=request.POST['belong'])
            form = ModelForm(request.POST, request.FILES, instance=obj)
            form_validated = form.is_valid()
            if form_validated:
                new_object = self.save_form(request, form, change=not add)
