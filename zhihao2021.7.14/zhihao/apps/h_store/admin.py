...
class InventoryAdmin(admin.ModelAdmin):
    ...
    actions = None
    list_filter = []
    exclude_display = ['belong','maker_id','id']
    exclude_filter = []
    search_fields = []
    ...
    def _changeform_view(self, request, object_id, form_url, extra_context):
        ...
        if request.method == "POST":
            ...
            try:
                ...
                pass
            request_copy['belong'] = str(request.user.belong)
        ...
        if add:
            ...
            id=obj.pk
            old_in_amount=Vaccine_in.objects.filter(id=id).first().in_amount
            ...
            if all_valid(formsets) and form_validated:
                ...
                if add:
                    ...
                    if v_name not in name_list:
                        Inventory.objects.create(belong=belong, goods=v_name, type=type_in, maker_id=maker_id,quantity=in_amount, f_date=f_date, operation=operation,out_time=timezone.now())
                    else:
                        inventory = Inventory.objects.filter(goods=v_name)
                        m_list = inventory.values_list('maker_id', flat=True)
                        if int(maker_id) not in m_list:
                            Inventory.objects.create(belong=belong, goods=v_name, type=type_in, maker_id=maker_id,quantity=in_amount, f_date=f_date, operation=operation,out_time=timezone.now())
                        else:
                            obj1 = Inventory.objects.filter(Q(goods=v_name) & Q(maker_id=maker_id))
                            num = obj1.values_list('quantity', flat=True)[0]
                            res = float(num) + float(in_amount)
                            obj1.update(quantity=res,out_time=timezone.now())
                    return self.response_add(request, new_object)
                else:
                    self.log_change(request, new_object, change_message)
                    new_in_amount=Vaccine_in.objects.filter(id=id).first().in_amount
                    in_amount_change=new_in_amount-old_in_amount
                    v_name=obj.v_name
                    maker_id=obj.maker_id
                    obj1 = Inventory.objects.filter(Q(goods=v_name) & Q(maker_id=maker_id))
                    num = obj1.values_list('quantity', flat=True)[0]
                    res = float(num) + float(in_amount_change)
                    obj1.update(quantity=res, out_time=timezone.now())
        ...
        model_name = str(self.model._meta).replace('.','_')
        change_data = self.change_view_data(request, object_id,model_name)
        context = {
    ...
    def _changeform_view(self, request, object_id, form_url, extra_context):
        ...
        if request.method == 'POST':
            ...
            if all_valid(formsets) and form_validated:
                ...
                tem = obj1.first()
                if add:
                    res = float(tem.quantity) - float(num)
                else:
                    old_num=Vaccine_out.objects.filter(id=object_id).first().num
                    num_change=float(num)-float(old_num)
                    res=float(tem.quantity) - float(num_change)
                if res >= tem.alert:
                    self.save_model(request, new_object, form, not add)
                    self.save_related(request, form, formsets, not add)
                    change_message = self.construct_change_message(request, form, formsets, add)
...
class Feedingin_Admin(admin.ModelAdmin):
    ...
    def _changeform_view(self, request, object_id, form_url, extra_context):
        ...
        if add:
            ...
            obj = self.get_object(request, unquote(object_id), to_field)
            id = obj.pk
            old_quantity = Feedingin.objects.filter(id=id).first().quantity
            if request.method == 'POST':
                if not self.has_change_permission(request, obj):
                    raise PermissionDenied
            else:
            ...
            if all_valid(formsets) and form_validated:
                ...
                if add:
                    ...
                    if f_name not in name_list:
                        Inventory.objects.create(belong=belong, goods=f_name, type=type, maker_id=maker_id,quantity=quantity, f_date=f_date, operation=operation,out_time=timezone.now())
                    else:
                        ...
                        if int(maker_id) not in m_list:
                            ...
                            obj1 = Inventory.objects.filter(Q(goods=f_name) & Q(maker_id=maker_id))
                            num = obj1.values_list('quantity', flat=True)[0]
                            res = float(num) + float(quantity)
                            obj1.update(quantity=res,out_time=timezone.now())
                    #######################
                    return self.response_add(request, new_object)
                else:
                    self.log_change(request, new_object, change_message)
                    new_quantity = Feedingin.objects.filter(id=id).first().quantity
                    ...
                    obj1 = Inventory.objects.filter(Q(goods=f_name) & Q(maker_id=maker_id))
                    num = obj1.values_list('quantity', flat=True)[0]
                    res = float(num) + float(quantity_change)
                    obj1.update(quantity=res, out_time=timezone.now())
                    return self.response_change(request, new_object)
            else:
                form_validated = False
...
class Feeding_outAdmin(admin.ModelAdmin):
    ...
    def _changeform_view(self, request, object_id, form_url, extra_context):
        ...
        if request.method == 'POST':
            ...
            if all_valid(formsets) and form_validated:
                belong = request.POST.get('belong')
                outbound_no = request.POST.get('outbound_no')
                f_name = request.POST.get('f_name')
                type = request.POST.get('type')
                maker_id = request.POST.get('maker_id')
                warehouse_num = request.POST.get('warehouse_num')
                delivery_time = '-'.join(request.POST.get('delivery_time').split('/'))
                out_purposes = request.POST.get('out_purposes')
                num = request.POST.get('num')
                out_staff = request.POST.get('out_staff')
                ...
                if add:
                    ...
                    old_num = Feeding_out.objects.filter(id=object_id).first().num
                    num_change=float(num)-float(old_num)
                    res=float(tem.quantity) - float(num_change)
                if res >= tem.alert:
                    self.save_model(request, new_object, form, not add)
